"""PS5 teleoperation for UR3e using damped least-squares (DLS) joint-space control.

External interface (for the user) is unchanged:
- PS5 controller inputs map to a desired TCP velocity (vx, vy, vz, wx, wy, wz).

Internally, this script:
- Reads current joint configuration q from the robot.
- Computes the UR3e geometric Jacobian J(q) at the TCP.
- Uses damped least-squares to map desired TCP velocity to joint velocity:
      qdot_task = J_dls * v_tcp
  where J_dls is a manipulability-dependent damped pseudoinverse.
- Adds a nullspace posture term that biases joints toward a comfortable posture.
- Enforces simple joint and workspace limits.
- Sends joint velocities with RTDE speedJ at a fixed control rate.
...
NULLSPACE_GAIN = 0.1  # how strongly we bias toward preferred posture

# Damping parameters for manipulability-based DLS
LAMBDA_MIN = 0.01
LAMBDA_MAX = 0.1
MANIP_HIGH = 0.02  # above this, very low damping
MANIP_LOW = 0.005  # below this, strong damping
...
def jacobian_ur3e(q: np.ndarray) -> np.ndarray:
    ...
    T_0_6, origins, z_axes = fk_ur3e(q)
    p_e = origins[-1]
...
def dls_joint_velocity(J: np.ndarray, v_tcp: np.ndarray, q: np.ndarray) -> np.ndarray:
    """Compute joint velocities using damped least-squares with nullspace posture bias."""
    w = manipulability(J)
    ...
    try:
        ...
        inv_term = np.linalg.pinv(JJt + lam2I)

    J_dls = J.T @ inv_term  # 6x6

    # Task-space joint velocity
    qdot_task = J_dls @ v_tcp

    # Nullspace term: bias toward preferred posture
    q_err = Q_PREFERRED - q
...
def enforce_joint_limits(q: np.ndarray, qdot: np.ndarray, dt: float) -> np.ndarray:
    ...
    for i in range(6):
        if q_next[i] < JOINT_LIMITS_MIN[i]:
            if qdot[i] < 0:
                # scale down magnitude so we don't move past the limit
...
print(
    """DLS joint-space control using PS5 controller (TCP-level teleop).
...
try:
    while True:
        ...
        if np.allclose(v_tcp, 0.0, atol=1e-4):
            ...
            # Compute Jacobian and DLS joint velocity
            J = jacobian_ur3e(q)
            qdot = dls_joint_velocity(J, v_tcp, q)
